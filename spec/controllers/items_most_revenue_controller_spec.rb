RSpec.describe Api::V1::ItemsMostRevenueController do
  before(:each) do
  ...
  describe "Get index" do
    it "shows a specified number of items ranked by revenue" do
      get :index, item_count: 2, format: :json
      items_revenue_hash = JSON.parse(response.body, symbolize_names: true)

      expect(response).to have_http_status(:success)
      expect(items_revenue_hash.count).to eq 2
      expect(items_revenue_hash.first[:name]).to eq "shoes"
    end
  end
end
